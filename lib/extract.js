    ...
    .end(function (err, response) {
      if (!err && response.statusCode == 200) {
       ...
       if ( !data.canonicalLink || !validator.isURL(data.canonicalLink) ){
        ...
        data.links = cleanLinks(parseResult.links);
...
function pull (txt, paragraphIndex, urlObj){
  ...
  var parser = new htmlparser.Parser({
      onopentag: function (name, attribs){
          ...
          if (name === 'a' && attribs.href){
            opentag[name]['href'] = attribs.href ? urlFix( attribs.href,  urlObj.host) : null;
          }
      },
  ...
  return {
    ...
    tags: _.chain(tags)
      .filter({ name:'a' })
      .filter(function (tag){ return tag && tag.href !== undefined; })
      .value()
  };
}
...
const cleanLinks = function (links) {
  return links.filter(function(link){
    return link.href.match(/\.(jpg|png|gif|bmp)$/i) === null;
  });
};
exports.urlFix = urlFix;
