...
const constants = require('./cookie');
...
exports.pageRequester = function (url, cb) {
  const queryLinkDataObject = { queryLink: url };
  ...
  request
    ...
    .end(function (err, response) {
      if (!err && response.statusCode == 200) {
        ...
        var text = response.text === undefined ? '' : response.text; // Checking that we have a string
        var data = extractor(text);
        data = _.assign(data, queryLinkDataObject);

       if ( !data.canonicalLink || !validator.isURL(data.canonicalLink) ){
  ...
  var parser = new htmlparser.Parser({
      onopentag: function (name, attribs){
          ...
          if (name === 'a' && attribs.href){
            opentag[name]['href'] = attribs.href ? urlFix( attribs.href,  urlObj) : null;
          }
      },
 ...
 */
exports.copyAssets = function (imageURL, faviconURL, uID, callback){
  async.parallel([
    ...
    function (cb){
      ...
      upload(faviconURL, faviconUID, function (err, url){
        if (err){
          let url = new URL(faviconURL);
          let guessURL = url.protocol + '//' + url.hostname + '/favicon.ico';
          upload(guessURL, faviconUID, function (err, urlTwo){
            if (err){
              cb(null, null);
            } else {
              cb(err, urlTwo);
...
exports.copyTwitterImage = function (twiterProfileURL, uID, callback) {
  const profileUID = 'profile_' + uID;
  upload(twiterProfileURL, profileUID, function (err, url){
    if (err){
      let url = new URL(profileUID);
      let guessURL = url.protocol + '//' + url.hostname + '/favicon.ico';
      upload(guessURL, profileUID, function (err, urlTwo){
        if (err){
          callback(err, null);
        } else {
          callback(err, urlTwo);
